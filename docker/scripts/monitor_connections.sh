...
while true; do
    ...
    echo "Tailscale status debug – logs command: $(tailscale logs)"
    echo "Tailscale status debug – 'Ready to accept connections': $(tailscale logs | grep 'Ready to accept connections')"
    echo "Tailscale status debug – 'Ready to accept connections'?: $(tailscale logs | grep 'Ready to accept connections' -q)"
